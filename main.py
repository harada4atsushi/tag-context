...
def splited_data(train):
    splited_data = []
    for datum in train:
        splited_data.append(split(datum[6]))
    return splited_data

train = np.array([
    [1, 0, 0, 0, 0, 0, 'こんにちは', 1],
    [0, 1, 0, 1, 1, 0, 'パソコンが壊れました。どうすればいいですか？', 2],
    [0, 1, 1, 0, 1, 1, 'パソコンを買いたいのですがどこで買おうかな？', 3],
])
...
X = np.array([
    [1, 0, 0, 0, 0, 0, 'こんにちは'],  # => 1
    [0, 1, 0, 0, 0, 0, 'パソコン'],  # => fail
    [0, 1, 1, 0, 1, 0, 'パソコンを買いたい。'],  # => 3
    [0, 1, 1, 0, 1, 1, 'パソコンを買いたい。どうすればいい？'],  # => 3
    [0, 0, 1, 0, 1, 0, 'ラーメンが食べたい。おすすめはどこですか？'],  # => fail
    [0, 0, 1, 0, 0, 1, 'エルメスのバッグが買いたいです'],  # => fail
    [1, 0, 0, 0, 0, 0, 'こんばんは'],  # => fail
])
